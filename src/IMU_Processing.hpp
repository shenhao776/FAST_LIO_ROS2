...
void ImuProcess::UndistortPcl(
  ...
  const double& imu_beg_time =
      rclcpp::Time(v_imu.front()->header.stamp).seconds();
  const double& imu_end_time =
      rclcpp::Time(v_imu.back()->header.stamp).seconds();
  ...
  // cout<<"[ IMU Process ]: Process lidar from "<<pcl_beg_time<<" to
  // "<<pcl_end_time<<", " \
...
void ImuProcess::Process(const MeasureGroup& meas,
  ...
  if (imu_need_init_) {
    ...
    if (init_iter_num > MAX_INI_COUNT) {
      ...
      // ROS_INFO("IMU Initial Done: Gravity: %.4f %.4f %.4f %.4f; state.bias_g:
      // %.4f %.4f %.4f; acc covarience: %.8f %.8f %.8f; gry covarience: %.8f
      // %.8f %.8f",\
      //          imu_state.grav[0], imu_state.grav[1], imu_state.grav[2],
      //          mean_acc.norm(), cov_bias_gyr[0], cov_bias_gyr[1],
      //          cov_bias_gyr[2], cov_acc[0], cov_acc[1], cov_acc[2],
