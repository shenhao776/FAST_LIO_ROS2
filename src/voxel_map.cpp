...
// [FIX] Global variable definition moved to source file
static int voxel_plane_id = 0;

// [FIX] Helper function replacing tf::createQuaternionMsgFromRollPitchYaw
geometry_msgs::msg::Quaternion createQuaternionMsgFromRollPitchYaw(double roll,
                                                                   double pitch,
                                                                   double yaw) {
  tf2::Quaternion q;
...
void calcBodyCov(Eigen::Vector3d& pb, const float range_inc,
  ...
  float deg_rad = degree_inc * M_PI / 180.0;
  direction_var << pow(sin(deg_rad), 2), 0, 0, pow(sin(deg_rad), 2);
...
// [FIX] ROS 2 parameter loading
void loadVoxelConfig(rclcpp::Node* node, VoxelMapConfig& voxel_config) {
  // Using declare_parameter with default values, assuming node is properly
  // namespaced if needed

  // Helper lambda for declaration and getting
  ...
  declare_and_get("publish.pub_plane_en", voxel_config.is_pub_plane_map_,
                  false);

  declare_and_get("lio.max_layer", voxel_config.max_layer_, 1);
  declare_and_get("lio.voxel_size", voxel_config.max_voxel_size_, 0.5);
  declare_and_get("lio.min_eigen_value", voxel_config.planner_threshold_, 0.01);
  declare_and_get("lio.sigma_num", voxel_config.sigma_num_, 3.0);
  declare_and_get("lio.beam_err", voxel_config.beam_err_, 0.02);
  declare_and_get("lio.dept_err", voxel_config.dept_err_, 0.05);

  std::vector<int64_t> default_layer_num = {5, 5, 5, 5, 5};
  if (!node->has_parameter("lio.layer_init_num")) {
    node->declare_parameter("lio.layer_init_num", default_layer_num);
  }
  std::vector<int64_t> layer_num_temp;
  node->get_parameter("lio.layer_init_num", layer_num_temp);
  voxel_config.layer_init_num_.clear();
  for (auto val : layer_num_temp)
    voxel_config.layer_init_num_.push_back(static_cast<int>(val));

  declare_and_get("lio.max_points_num", voxel_config.max_points_num_, 50);
  declare_and_get("lio.max_iterations", voxel_config.max_iterations_, 5);

  declare_and_get("local_map.map_sliding_en", voxel_config.map_sliding_en,
                  false);
  declare_and_get("local_map.half_map_size", voxel_config.half_map_size, 100);
  declare_and_get("local_map.sliding_thresh", voxel_config.sliding_thresh, 8.0);
}
  ...
  for (int iterCount = 0; iterCount < config_setting_.max_iterations_;
    ...
    if (effct_feat_num_ > 0) {
      ...
      for (int i = 0; i < effct_feat_num_; i++) {
        ...
        M3D var;
        var = state_.rot_end * extR_ * ptpl_list_[i].body_cov_ *
              (state_.rot_end * extR_).transpose();
        ...
        Hsub_T_R_inv.col(i) << A[0] * R_inv(i), A[1] * R_inv(i),
            A[2] * R_inv(i), ptpl_list_[i].normal_[0] * R_inv(i),
      ...
      MD(DIM_STATE, DIM_STATE) && K_1 =
          (H_T_H.block<DIM_STATE, DIM_STATE>(0, 0) +
           state_propagat.cov.block<DIM_STATE, DIM_STATE>(0, 0).inverse())
              .inverse();
      ...
      if (!EKF_stop_flg &&
        ...
        state_.cov = (I_STATE.block<DIM_STATE, DIM_STATE>(0, 0) -
                      G.block<DIM_STATE, DIM_STATE>(0, 0)) *
                     state_propagat.cov.block<DIM_STATE, DIM_STATE>(0, 0);

        position_last_ = state_.pos_end;
        V3D euler_cur = state_.rot_end.eulerAngles(2, 1, 0);
        geoQuat_ = createQuaternionMsgFromRollPitchYaw(
            euler_cur(0), euler_cur(1), euler_cur(2));
        EKF_stop_flg = true;
      }
    } else {
      std::cout << ANSI_COLOR_YELLOW
                << "[ LIO ] No effective features. Skipping EKF update."
...
void VoxelMapManager::build_single_residual(pointWithVar& pv,
  ...
  if (current_octo->plane_ptr_->is_plane_) {
    ...
    float dis_to_center_sq = (plane.center_ - p_w).squaredNorm();
    float range_dis = sqrt(dis_to_center_sq - dis_to_plane * dis_to_plane);
    ...
    if (range_dis <= radius_k * plane.radius_) {
      ...
      double plane_var_term = J_nq * plane.plane_var_ * J_nq.transpose();
      double pv_var_term = plane.normal_.transpose() * pv.var * plane.normal_;
      double sigma_l = plane_var_term + pv_var_term;

      double distance_threshold = sigma_num * sqrt(sigma_l);
      ...
      if (dis_to_plane < distance_threshold) {
        ...
        if (this_prob > prob) {
          prob = this_prob;
          // pv.normal = plane.normal_; // Removed because pointWithVar doesn't
          // contain normal
    ...
    if (current_layer < max_layer) {
      for (size_t leafnum = 0; leafnum < 8; leafnum++) {
        if (current_octo->leaves_[leafnum] != nullptr) {
          build_single_residual(pv, current_octo->leaves_[leafnum],
                                current_layer + 1, is_sucess, prob,
                                single_ptpl);
        }
      }
    }
...
void VoxelMapManager::pubVoxelMap(const rclcpp::Time& stamp) {
...
void VoxelMapManager::pubSinglePlane(
  ...
  plane.color.g = rgb(1);
  plane.color.b = rgb(2);
  // plane.lifetime = rclcpp::Duration(0, 0); // Lifetime 0 means forever
...
void VoxelMapManager::mapSliding() {
  if ((position_last_ - last_slide_position).norm() <
    ...
    if (ANSI_COLOR_RED) std::cout << ANSI_COLOR_RED;
    std::cout << "[DEBUG]: Last sliding length "
              << (position_last_ - last_slide_position).norm();
    if (ANSI_COLOR_RESET) std::cout << ANSI_COLOR_RESET;
    std::cout << "\n";
    return;
  }
  ...
  if (ANSI_COLOR_RED) std::cout << ANSI_COLOR_RED;
  std::cout << "[DEBUG]: Map sliding using " << t_sliding_end - t_sliding_start
            << " secs";
  if (ANSI_COLOR_RESET) std::cout << ANSI_COLOR_RESET;
  std::cout << "\n";
  return;
