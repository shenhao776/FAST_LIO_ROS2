...
// [FIX] Move static variable to cpp or use extern to avoid duplication in
// headers extern int voxel_plane_id;

// [FIX] Define pointWithVar structure which is used but not defined in the
// snippet
struct pointWithVar {
  ...
  float intensity;  // Added for compatibility with MapUpdater
};
...
struct PointToPlane {
  ...
  M3D body_cov_;  // M3D defined in common_lib.h
  int layer_;
  double d_;
  double eigen_value_;
  bool is_valid_;
...
// [FIX] Updated to use rclcpp::Node
...
class VoxelMapManager {
 public:
  ...
  // [FIX] Initialization method to set publisher (since constructor is minimal)
  ...
  // [FIX] Need ROS context (Time) for publishing
